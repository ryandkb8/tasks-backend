...
class TasksDaoSpec extends PlaySpec with GuiceOneAppPerSuite with BeforeAndAfter with TestUtil {
  ...
  "find all tasks due in specific dates" in {
    ...
    val taskForm1 = TaskForm(randomString, "My first task", date1, None)
    val taskForm2 = TaskForm(randomString, "My first task", date1, None)
    val taskForm3 = TaskForm(randomString, "My first task", date2, None)
    val taskForm4 = TaskForm(randomString, "My first task", date3, None)
